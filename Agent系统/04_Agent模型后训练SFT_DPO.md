...
Step 4：质量过滤（三层）
  ...
  ├── LLM-as-Judge 推理链评分（Step 2.5，新增）
  │   调用强LLM对每条对话打分（0-10），低于6分的样本在打包前丢弃
  │   评分维度：工具调用顺序 / 参数来源合法性 / Thought与Action一致性 / 结论完整性 / 危险操作识别
  │   预计过滤额外 10-20% 的"格式正确但推理逻辑错误"样本
  │   → 代码：generate_sft_data.py 中的 filter_by_quality()
  ├── 一致性校验：最终结论与工具返回数据是否逻辑自洽
  └── 人工抽检：随机抽取5%样本，专家审核逻辑正确性
```

**为什么要加 LLM-as-Judge 这一层**：

Pydantic 只能发现格式问题，无法识别"格式正确、逻辑错误"的样本，例如：
- 先调用 `simulation_runner` 再调用 `test_case_query`（工具顺序颠倒）
- Thought 说"需要查询切换用例"但 Action 却调用了 `metrics_collector`
- 凭空捏造工具返回值中的 `session_id`，而非从上一步获取

这类样本进入训练集后，模型会把错误的推理模式当作正确示例学习，危害比格式错误更难察觉、更难纠正。
...
| **微调代价** | 32B LoRA 约18小时，72B 需要 50+ 小时，迭代速度差异显著 |

**为什么选 LoRA 而不是全参微调（Full Fine-tuning）**：
...
LoRA（Low-Rank Adaptation）：
  优点：
  ├── 显存：基座模型以 BF16 加载，8×A100 ZeRO-3 分片后每卡仅需约 10GB 存放参数
  ├── 效果：在充分数据下，效果接近全参（通常 95%+ 的性能）
  ├── 精度：BF16 全精度权重保留，无量化误差，适合对数值精度有要求的推理任务
  └── 安全：原始权重冻结，通用能力不受损
  缺点：性能上限低于全参（可接受）；相比 QLoRA 显存占用更高，需要足够的 GPU 资源
```
...
**为什么不使用 QLoRA（4bit 量化 LoRA）**：

本项目使用 8×A100 80G（总显存 640GB），完全可以承载 32B 模型的 BF16 全精度加载，无需通过量化压缩显存。QLoRA 的 NF4 量化会引入量化误差，在工具调用格式精确性和数值参数准确性上略有损失。在算力充足的情况下，LoRA 是比 QLoRA 更优的选择。
...
model = AutoModelForCausalLM.from_pretrained(
    "Qwen/Qwen3-32B",
    torch_dtype=torch.bfloat16,   # 全精度BF16，数值稳定且精度无损
    device_map="auto",            # ZeRO-3 接管分片，此处先 auto 初始化
)

# 训练超参数
training_args = TrainingArguments(
    num_train_epochs=3,               # 3个Epoch：实验发现2个Epoch欠拟合，4个开始过拟合
    per_device_train_batch_size=2,    # 8×A100，每卡 batch_size=2
...
**显存占用估算（8×A100 80G，ZeRO-3）**：

| 组件 | 显存大小 | 说明 |
|:---|:---|:---|
| 基座模型参数（BF16） | ~64GB 总计 / ~8GB 每卡 | ZeRO-3 分片到8卡 |
| LoRA 可训练参数（r=64） | ~1.6GB 总计 | 仅占基座的 ~2.5% |
| 优化器状态（AdamW） | ~10GB 每卡（CPU offload） | 卸载至 CPU |
| 激活值（gradient checkpointing） | ~15GB 每卡 | 重算激活值降低峰值 |
| **峰值每卡显存** | **约 30-35GB** | 80GB 卡有充足余量 |

**`r=64` 的选择逻辑**：

通信测试场景的任务多样性高（切换、干扰、容量、信道等），需要较高的 LoRA 秩来捕捉足够的任务变化。实验对比（BF16 全精度加载，显存增量为 LoRA 参数本身）：

| r 值 | 格式合规率 | 术语准确率 | LoRA 参数显存增量 |
|:---|:---|:---|:---|
| 16 | 91.2% | 83.4% | +0.5GB |
| 32 | 94.8% | 88.1% | +1.0GB |
| **64** | **96.7%** | **91.3%** | **+2.0GB** |
...
`r=64` 是性能和显存的最优平衡点，从 64 到 128 的提升微乎其微但 LoRA 参数量翻倍。由于采用 BF16 全精度，总显存瓶颈在基座模型本身（~64GB），LoRA 参数增量相对基座可忽略不计。
...
DPO 阶段同样使用 LoRA，在 SFT 模型的基础上加载新的 LoRA 适配器进行偏好优化，SFT 阶段训练的 LoRA 权重保持冻结作为 Reference。

```python
# DPO 阶段的 LoRA 配置（与 SFT 相同结构，独立初始化新适配器）
dpo_lora_config = LoraConfig(
    r=32,                    # DPO 阶段秩可适当降低，偏好对齐任务相对简单
    lora_alpha=64,
    lora_dropout=0.05,
...
`confidence_score < 0.65` 触发 HITL 的阈值是静态值，但每次模型更新后其置信度分布会发生漂移：
- SFT 后模型倾向于输出更高的置信度
- DPO 后模型对危险场景会主动降低置信度

阈值不随模型更新会导致 HITL 误触发过多（阈值偏高）或漏报不确定性（阈值偏低）。

**校准方法**（已集成进 `train_sft.py`，训练结束后自动运行）：

```
在验证集上遍历置信度区间（步长 0.05）：

  [0.00, 0.05)  错误率 38%  → 需要 HITL，跳过
  [0.05, 0.10)  错误率 31%  → 需要 HITL，跳过
  ...
  [0.55, 0.60)  错误率 19%  → 需要 HITL，跳过
  [0.60, 0.65)  错误率 12%  ← 低于容忍上限（15%），确定为新阈值
  → 新阈值 = 0.60
```

校准逻辑：找"错误率首次降至 15% 以下"的置信度区间下界，作为新的 HITL 触发阈值。若新阈值与旧值差距 > 0.1，输出提示并要求同步更新 `nodes.py` 中的 `CONFIDENCE_THRESHOLD` 常量。

| 校准时机 | 操作 |
|:---|:---|
| SFT 训练完成后 | 自动运行，输出建议阈值 |
| DPO 训练完成后 | 再次运行，DPO 会进一步改变置信度分布 |
| 每周增量微调后 | 纳入飞轮的标准流程，每次都重新校准 |
...
不是。r 值控制低秩矩阵的表达能力，但存在收益递减和参数量膨胀的权衡。在本项目中实验发现 r=64 是最优点，r=128 性能几乎没有提升但 LoRA 参数量翻倍。建议从 r=16 开始实验，逐步增大直到验证集性能不再提升。

注意：使用标准 LoRA（BF16 全精度）时，显存的主要瓶颈是基座模型本身（32B × 2 bytes ≈ 64GB），LoRA 参数增量（r=64 约 2GB）相对基座可忽略不计。因此 r 值对显存的影响远小于对 QLoRA 的影响，可以适当选择更大的 r 值而不用过分担心显存压力。
...
初期 HITL 数据 < 200 条时，启用 **冷启动模式**（已系统化集成进 `prepare_dpo_data.py`）：

```
冷启动流程（--cold_start 模式）：

Step 1：从高风险场景模板中取一批 prompt
  ├── 全量压测（concurrency 越界）
  ├── 全局重置（reset_all/force_reboot）
  ├── 无终止条件循环
  └── 受限频段使用 等8类场景

Step 2：调用强 LLM（GPT-4o）为每个 prompt 生成两个版本
  ├── unsafe_version：包含危险操作的回复
  └── safe_version：遵守安全约束的等价回复

Step 3：规则评分器交叉验证（纯本地，不依赖 API）
  ├── 检查 unsafe 是否确实包含禁止模式（reset_all / concurrency>100 等）
  ├── 检查 safe 是否确实不包含上述模式
  └── 两侧评分相同的样本 → 区分度不够，丢弃

Step 4：输出可供专家抽检的 JSONL，建议抽检 10%，确认后入库
```
...
python prepare_dpo_data.py \
  ...
  --output_file data/dpo_cold_start.jsonl
```

**与合成数据（`--use_synthetic`）的区别**：合成数据是对固定模板的文本随机变体，多样性有限；冷启动数据让强 LLM 自由生成两种安全级别的版本，场景覆盖更广，且经过规则验证，质量更可靠。

等 HITL 积累到 1000+ 条后，冷启动数据逐步替换为真实偏好数据，两者混合比例建议：`冷启动 : HITL = 3 : 7`。
