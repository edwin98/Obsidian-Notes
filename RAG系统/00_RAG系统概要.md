...
## Q4: Redis 在这个系统里究竟起到了什么决定性的拉升作用，以至于被称为“多级状态海马体”？
**解答**：
没有它，大模型 API 的网关瞬间就会被昂贵的 Token 计费和并发击破。
1. **防止缓存击穿与雪崩**：对于热点的查询（比如公司出台某项重发政策，早晨 9 点上万人进来问同一个词），通过 Redis 分布式锁，只放一台实例穿透去等待大模型 10 秒慢推理，其他 9999 会话全部自旋等待后复用这一条 `setex` 答案，拯救了财务账单和后端底座。
2. **多轮对话动态滑动窗口切割**：为了让模型拥有多轮记忆又不超 Token，我们利用 Redis 天然的 `RPUSH` 与 `LTRIM` 组合技。只要用户对话，我们便在内存的最末端切掉过老的上下文，以接近 $O(1)$ 的开销维持住最高效的上下文轮次（比把 JSON 拉出来用程序截断好了一万倍性能）。
