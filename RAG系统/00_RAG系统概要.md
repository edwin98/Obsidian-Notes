...
- **亮点**：在核心架构上做了**绝对的物理与逻辑冷热解耦隔离**。在线实时并发交由 FastAPI 异步事件循环底座承载，配合 **LangGraph 状态机**进行多轮对话状态的强隔离管理（基于 Thread ID 实现并发多租户会话隔离，天然封堵乱序串键风险）；离线长周期动作被 Kafka 消息大坝完全拦截剥离，交给 CPU/GPU 分流的专有 Worker 队列凭自身算力上限平滑消费。这套"推拉结合、算力解耦"的架构是可用性的底盘。
...
- **输入拼接与深层交互层 (Self-Attention)**：
  其内部结构的本质是将用户查询与候选文档拼接成一整个序列：`[CLS] Query [SEP] Document [SEP]`，一同送入底层 Transformer（如 BERT 或 RoBERTa 的核心块）。通过多头自注意力机制 (Multi-Head Self-Attention)，Query 中的每一个词（如“5G”）和 Document 中的每一个词（如“基站”、“掉线”）都在网络的每一层发生了密集的**词级交叉感知（Cross-Attention）**。
- **打分输出分类层 (Classification Head)**：
  经过几十层的前向传播后，最顶层的特殊占位符 `[CLS]` Token 吸纳了整体序列的细粒度交叉融合语义特征。将其提取出来送入接驳的**反向传播线性特征层 (Linear Layer / MLP)**，计算后输出一维浮点数逻辑值（Logits），通常再通过 Sigmoid 激活，最终得出一个介于 0 到 1 之间代表“极度不相关”到“完美匹配”的相关度连续变量得分。
...
- **微调效果收益 (Metrics Lift)**：经历上述训练循环后，模型对待极短词汇缩写变异的“视力”被大幅度拉升。在前 10 名命中率在含有复杂长尾词集的评测基准上的 NDCG@10 指标猛增约 23%，彻底填补平了“粗搜打得准，但重排看瞎眼”的性能峡谷断层。
