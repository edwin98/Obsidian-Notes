...
以 **Qwen2.5-32B-Instruct**（推荐选型，见下节）128K 上下文窗口为例：
...
| ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- |
| Qwen2.5-7B-Instruct | 7B | 128K | ⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ~14 GB (fp16) | 🟡 备选（资源受限时） |
| Qwen2.5-14B-Instruct | 14B | 128K | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ~28 GB (fp16) | 🟡 中档 |
| **Qwen2.5-32B-Instruct** | **32B** | **128K** | **⭐⭐⭐⭐⭐** | **⭐⭐⭐⭐⭐** | **⭐⭐⭐⭐** | **~64 GB (fp16)** | **✅ 首选推荐** |
| Qwen2.5-72B-Instruct | 72B | 128K | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ~144 GB (fp16) | 🔵 备选（富裕资源） |
| Qwen2.5-7B-Instruct-1M | 7B | **1M** | ⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐ | ~14 GB | 🔵 未来超长文档场景 |

### 最终推荐：Qwen2.5-32B-Instruct

**推荐理由（逐条分析）：**

**① 综合性能越级表现**
Qwen2.5-32B 的综合性能基准分显著超越上代 Qwen2-72B，即：以 32B 的硬件成本获得了此前需要 72B 才能匹配的输出质量。这在算力紧张的私有化部署场景中意义重大。

**② 指令遵循能力达到实用天花板**
我们的 System Prompt 包含极为复杂的行为约束（禁止捏造、强制 citation、角色扮演），这对模型的指令对齐（Instruction Following）能力要求极高。32B 在此维度上得分与 72B 几乎持平，远超 7B/14B。

**③ 128K 上下文窗口完美覆盖需求**
根据上一节的分析，单次请求 Token 上限约 18,000–20,000（极端情况）。32B 的 128K 窗口提供了超过 6 倍的余量，即便未来引入更多检索文档也绰绰有余。

**④ 推理速度满足 TTFT < 3s 要求**
在 vLLM + PagedAttention 加速下，Qwen2.5-32B 在 2xA100（80G）环境中可实现 30–50 tokens/s 的生成速度，TTFT 稳定控制在 2–2.5s，满足系统的严格延迟目标。

**⑤ 显存需求可控，支持私有化部署**
fp16 约需 64GB 显存，2 张 A100(80G) 或 3 张 A30(24G) 即可轻松运行（亦可 AWQ/GPTQ 4-bit 量化至 ~18GB 单卡运行，轻微质量损耗可接受）。

> [!NOTE]
> **备选方案说明：**
> - **资源受限场景（仅有单张 A100-40G）**：选用 `Qwen2.5-14B-Instruct`，4-bit 量化后约 7GB，牺牲约 10–15% 的指令遵循精度。
> - **追求极致质量（不计成本）**：选用 `Qwen2.5-72B-Instruct`，搭配 4 张 A100-80G 即可；在企业核心技术文档场景下，质量提升约 8–12%。
> - **未来超长文档原生处理（无需切片）**：关注 `Qwen2.5-14B-Instruct-1M`，其 100 万 token 上下文窗口将为免切片整书理解提供可能。
