...
### 1. Reranker 模型微调 (Fine-Tuning) 技术过程
通用的开源 Rerank 模型（如 gte-reranker）在泛领域表现出色，但面对通信专有黑话（如“A3事件”、“Xn切换”），很容易由于未见过这些特定搭配而给出极低的交叉相似度打分。为此，我们在底座基础上实施了领域交叉微调：
- **数据对构造机制 (Triplet Generation)**：不采用纯人工标注（极为低效）。我们通过 LLM 结合线上隐性反馈进行半自动化对齐挖掘。构造了大量形如 `<Query, Positive_Document, Hard_Negative_Document>` 的对比学习三元组对：
  - **正例 (Positive)**：取用原 QA 数据集中提取的正例或历史高命中工单日志。
  - **难负例 (Hard Negative) 挖掘策略**：这是微调成功的关键。为了防范“字面相近但逻辑相左”造成的误伤，我们利用系统第一阶段基于 BM25 词法检索拦截下的**高词频重叠但逻辑迥异**片段。比如问题是：“5G SIB1 配置参数”，系统将“4G SIB1 配置参数”或者“5G SIB2 配置参数”塞给微调模型作为 $Hard\_Negative$，进而强力逼迫 Cross-Encoder 在 Attention 层领悟对细微数字序号与特殊字符的高度敏锐鉴别权重。
- **训练手段与 Loss 优化**：使用 `CrossEntropyLoss` 或者 `MarginMSELoss` 进行基于知识蒸馏的微调。输入拼接格式规定为 `[CLS] Query [SEP] Document [SEP]`。为节约资源与防灾难性遗忘，采用 LoRA 冻结底部大部分 Encoder 层，仅放开前端层与最终线性分类层特征进行重训学习（PEFT 方法）。
- **微调效果收益 (Metrics Lift)**：对重排模型注入这种强化感知后，在前 10 名命中率在复杂的长尾专有提问数据集上的 NDCG@10 指标猛增约 23%，大幅改善“粗搜进前 3000，重排掉出前 10”的尴尬困境。
