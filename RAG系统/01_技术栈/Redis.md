...
## 0. 技术原理与背景信息
*   **技术原理**：Redis 是一款全内存操作的键值对数据库，由于直接操作内存，规避了缓慢的磁盘 I/O。它的网络事件处理采用了单线程的 I/O 多路复用机制（如 epoll），这意味着它的每条基础命令天生是原子性的。它内置了极度优化的底层数据结构（如哈希表、跳跃表 SkipList、压缩列表等），对不同的使用场景能自动做到内存的高效利用与毫秒级的性能突破。
*   **背景信息（为什么需要它）**：大模型生成接口调用单次价格昂贵且响应速度极慢（动辄数秒到数十秒）。如果同一个热门问题被反复抛给大模型分析，整个 RAG 系统的算力和财力会瞬间被耗尽。同时，大模型作为无状态的引擎，没有记忆力。基于以上痛点，我们迫切需要一个速度极快、支持过期淘汰（TTL）的内存组件，来充当历史多轮对话的“记忆海马体”（存下 Session），并担负直接拦截重复问题响应结果的“缓存防波堤”。
...
### Q1: Redis 为什么单线程也能这么快？
**回答**：
1. **纯内存操作**：完全规避了磁盘 I/O，是纳秒级/微秒级响应的基础。
2. **I/O 多路复用 (epoll/kqueue)**：只需一个主线程去监听成千上万个客户端的 Socket 连接，当有网络事件（读/写）准备就绪时再执行，避免了频繁阻塞等待。
3. **避免上下文切换**：单线程免去了多线程操作导致的频繁 CPU 上下文切换损失。
4. **无锁化设计**：单线程按顺序处理指令，不用加锁避免了竞争死锁。配合底层极其精简高效的数据结构（如跳表、SDS）。

### Q2: 什么是缓存穿透、缓存击穿、缓存雪崩？分别怎么防？
**回答**：
1. **击穿 (Breakdown)**：某个热门热点 Key 失效瞬间，巨大并发请求砸向后端（如查询耗时的 RAG 大模型）。
   - **防范**：互斥锁（使用 Redis 分布式锁，强制排队，第一个去库里拿数据重建缓存）。
2. **雪崩 (Avalanche)**：同一时刻，Redis 里大量的 Key 批量过期失效，或者 Redis 宕机，流量全部冲向底层数据库。
   - **防范**：对 TTL 设置随机抖动偏差（Jitter），防止集中失效；建设 Redis 哨兵/集群高可用；对后端服务做**限流熔断**保护。
3. **穿透 (Penetration)**：黑客发起大量恶意请求查询**根本不存在的数据**，由于不存在，也就永远不会放入缓存，致使所有请求直接穿透 Redis 砸向 DB。
   - **防范**：缓存空对象（如存值 `"{}"` 并设极短 TTL）；上**布隆过滤器 (Bloom Filter)** 在请求进入缓存前过滤掉一定不存在的 Key。

### Q3: RDB 与 AOF 持久化有什么区别？RAG 怎么选型？
**回答**：
- **RDB (Redis Database)**：隔一段时间进行一次全量内存快照生成二进制文件。优点是恢复极快，缺点是宕机会丢失最近一次快照后修改的数据，对数据完整性有一定折损。
- **AOF (Append Only File)**：以纯文本方式追加记录每一条写命令（如 `set`）。优点是安全性极高（比如 `always` 策略几乎不丢数据），缺点是文件体积大、系统恢复慢。
- **选型建议**：在 RAG 中，对于“缓存类型”的数据（随时能重新推理得来，只为加速），一般**只开 RDB 甚至都不开持久化**。但是针对“Session 历史多轮对话”，为了防止集群故障导致用户前言搭后语彻底丢失，建议**开启 AOF (每秒刷盘策略)**，以平衡性能与强安全性。
